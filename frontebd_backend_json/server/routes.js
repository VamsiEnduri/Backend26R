...
router.put("/users/:userId",(req,res)=>{
     let updatableId=req.params.userId;
     console.log(updatableId)
     console.log(req.body,"put req ncoming data in req.body")
     const {userId,name,age,role,info}=req.body;

     let data=readDataFile();
     const userFound=data.users.findIndex((x)=>x.userId === parseInt(updatableId))
     console.log(userFound)

     data.users[userFound]={userId,name,age,role,info};
     writeDataFile(data)
     res.writeHead(200,{"content-type":"application/json"})
...
// let data=readDataFile()
//      let findIndex=data.users.findIndex(x=>x.userId === updatableId);
...
router.put("/admins/:userId",(req,res)=>{
    let updatableId=req.params.userId;
    console.log(updatableId)
    console.log(req.body,"put req ncoming data in req.body")
    const {userId,name,age,role,info}=req.body;
    ...
    let data=readDataFile();
    const adminFound=data.admins.findIndex((x)=>x.userId === parseInt(updatableId))
    // console.log(userFound)

    data.admins[adminFound]={userId,name,age,role,info};
    writeDataFile(data)
    res.writeHead(200,{"content-type":"application/json"})
    res.write(JSON.stringify({message:"user updated successfully!!"}))
    return res.end()
